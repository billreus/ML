...
以两列以上进行分组`gruop_id = df.groupby(['ID','bill'])`
...
### 用固定值代替
用0代替缺失值`df.fillna(0)`
用一个字符串代替缺失值`df.fillna('missing')`
用前一个数据代替`df.fillna(method='pad')`
用后一个数据代替`df.fillna(method='bfill')`
用limit限制每列代替NaN的数目`df.fillna(method='pad', limit=1)`

### 用统计值代替
用平均数代替`df.fillna(df.mean())`
可以选择哪一列进行缺值处理`df.fillna(df.mean()['one':'two'])`
...
### 排序sort_value()
按照指定标签排序`df2 = df.sort_value(by='ID')`
